...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const setError   = (i, s, m) => { if(i){ i.classList.add('input-error'); i.setAttribute('aria-invalid','true'); } if(s) s.textContent = m; };
    const clearError = (i, s)    => { if(i){ i.classList.remove('input-error'); i.setAttribute('aria-invalid','false'); } if(s) s.textContent = ''; };

    const vNombre  = () => { if(!fNombre) return true; if(fNombre.value.trim().length < 3){ setError(fNombre,eNombre,'Ingresá al menos 3 caracteres.'); return false; } clearError(fNombre,eNombre); return true; };
    const vEmail   = () => { if(!fEmail) return true; if(!regex.test(fEmail.value.trim())){ setError(fEmail,eEmail,'Ingresá un email válido.'); return false; } clearError(fEmail,eEmail); return true; };
    const vMensaje = () => { if(!fMensaje) return true; if(fMensaje.value.trim().length < 10){ setError(fMensaje,eMensaje,'El mensaje debe tener al menos 10 caracteres.'); return false; } clearError(fMensaje,eMensaje); return true; };

    fNombre?.addEventListener('blur', vNombre);
    fEmail?.addEventListener('blur', vEmail);
    fMensaje?.addEventListener('blur', vMensaje);

    form?.addEventListener('submit', e => {
        e.preventDefault();
        const ok = vNombre() & vEmail() & vMensaje();
        if (ok) {
    ...
    document.addEventListener('click', e => {
        ...
        if (eduImg) { e.preventDefault(); abrirLightbox(eduImg.src, eduImg.alt); return; }

        // Imagen dentro del wrapper de certificaciones
        const certImg = e.target.closest('.cert-img-wrap img');
        if (certImg) { e.preventDefault(); abrirLightbox(certImg.src, certImg.alt); return; }
        ...
        if (certWrap) {
            ...
            if (img) { e.preventDefault(); abrirLightbox(img.src, img.alt); }
        }
    });
